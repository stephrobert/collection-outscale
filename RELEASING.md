...
**Nothing is published from this repository by itself.** The Galaxy token and
the tag belong to the maintainer; the workflow only runs when a tag is pushed,
and the `galaxy` environment holds the token.
...
explicitly, and the collection README announces it.
...
mise run readme           # the derived blocks and links follow the new version
...
it looks: the compatibility table, the versioning examples and the links
published next to the version are derived from it, so skipping this step ships
...
**The version commit goes through a pull request like every other commit.** The
ruleset on `main` requires it and refuses a direct push, and that is the right
shape: what ships must come from the branch that ran the required checks.

Once it is merged, and **only** then, because a squash merge rewrites the
commit and a tag placed before would point at something `main` does not
contain:

```bash
git switch main && git pull
git tag -s 0.2.0 -m "stephrobert.outscale 0.2.0"
...
1. `mise run release:check`, the gate. It refuses six things, and each one would
   ...
   * a tag that is not on `main`. What ships must come from the branch that was
     reviewed; the workflow triggers on the tag, so nothing else checks it;
   ...
   * a blocking documentation defect. A page that shows a filter the API
     refuses, or a returned field nobody described, is published forever. The
     same gate runs on every pull request, so this one is the belt after the
     braces.
2. `mise run check`, what every pull request passes;
3. `mise run sanity`, what Ansible says of the produced file;
4. `mise run package`, which builds the archive, checks its contents against
   what `plugins/` holds on disk, installs it into a throwaway directory and
   asks `ansible-doc` for its documentation. A file present in an archive is not
   a module Ansible can load;
...
filter**, and the difference cost the sibling repository one run. A workflow
filter accepts `+` as a quantifier; a deployment policy filters with `fnmatch`,
where `+` is a literal character. The patterns are therefore
`[0-9]*.[0-9]*.[0-9]*` and `v[0-9]*.[0-9]*.[0-9]*`. Copying the workflow's
patterns across produced a policy that could never match: the tag triggered the
workflow, and the job was rejected two seconds later, before its first step,
with `Tag "0.2.0" is not allowed to deploy to galaxy`.
...
```bash
ansible-galaxy collection install stephrobert.outscale:==0.2.0 -p /tmp/verif
ansible-doc -M /tmp/verif/ansible_collections/stephrobert/outscale/plugins/modules \
  stephrobert.outscale.vm_info
```

The page must explain the module on its own: what it reads, which filter keys
it accepts, what each returned field is, and an example that can be copied as
is. That is the criterion `mise run docs:quality` measures, and the reason it
gates both every pull request and the release.

## Publishing by hand

Only if the workflow cannot run. The two things learnt in the sibling
repository the day it was done by hand, written here rather than rediscovered:
...
* **an option that disappears is announced** the same way. A parameter the
  ...
  is what surfaces it;
* **`OSC_ENDPOINT_API` stays honoured end to end.** It is what lets you test
  against a local emulator without credentials, and no change may take it away.
