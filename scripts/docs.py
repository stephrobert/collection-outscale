...
* **`lint-collection-docs`** juge la documentation, et il voit une classe de
  défauts qu'`ansible-test sanity` laisse passer. Mesuré sur
  `collection-scaleway` : sur un module qui écrit `M(ns.coll.module_inexistant)`,
  `validate-modules` sort en 0 sans rien signaler, et `antsibull-docs` sort en 3
  en nommant la référence, dans `DOCUMENTATION` comme dans `RETURN` ;
...
**Le même piège que pour `ansible-test`.** Sur une collection qui ne contient
aucun plugin, `lint-collection-docs` sort en 0 sans rien dire. Un vert qui n'a
rien examiné est indiscernable d'une documentation irréprochable, donc ce
script exige de savoir ce qu'il a examiné, avant et après.
