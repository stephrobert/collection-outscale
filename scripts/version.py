...
un nombre recopié de plus, et `collection-scaleway` en a corrigé quatre en
deux jours. Ce qu'une version promet est déjà écrit dans les fragments de
changelog : une rupture, un ajout, ou une correction. Il n'y a donc rien à
décider, seulement à lire.
...
Pourquoi les fragments et non les sujets de commit, comme le fait `cz bump`
ailleurs : Ansible lit `version:` dans `galaxy.yml` et nomme l'archive d'après
lui, donc un fichier porte forcément le numéro ; le changelog appartient à
antsibull, dont `lint:changelog` vérifie le format ; et les commits de ce
dépôt sont en français, quand un fragment est publié, donc en anglais.
...
#: gagne, du plus fort au plus faible. `release_summary` n'implique rien : il
#: décrit la version, il ne la change pas.
IMPLICATIONS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    ("correctif", ("bugfixes", "security_fixes", "known_issues", "trivial")),
)
...
def composer(version: str) -> None:
    """Fait absorber les fragments par `antsibull-changelog`, via le pilote du dépôt."""
...
def main(argv: list[str]) -> int:
    ...
    for _echelon, declencheurs in IMPLICATIONS:
        for section in (s for s in declencheurs if s in sections):
    ...
    print(
        ...
        "Reste à faire suivre les liens versionnés, relire le diff, committer,\n"
        "puis taguer une fois la pull request fusionnée :\n"
        "  mise run readme\n"
        f"  git tag -s {cible} -m '{collection.fqcn} {cible}'"
    )
    return 0
