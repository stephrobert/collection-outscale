...
options:
  filters:
    description: 'One or more filters. Accepted keys: C(ClientTokens), C(NatServiceIds), C(NetIds),
      C(States), C(SubnetIds), C(TagKeys), C(TagValues), C(Tags).'
    type: dict
...
MODULE = InfoModule(
    resource="nat_service",
    operation=Operation(
        id="ReadNatServices",
        method="ReadNatServices",
        body_params={"filters": "Filters"},
        payload_field="NatServices",
        is_list=True,
