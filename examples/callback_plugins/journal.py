...
`non_emules`, parce qu'ils passent par `set_fact` et qu'un `set_fact` est un
résultat de tâche comme un autre. Rien à ajouter au playbook.
...
class CallbackModule(CallbackBase):
        ...
        self._taches.append(
            {
                ...
                # collection : feint répond 501 avec ce message, et le runtime
                # le rend dans `msg`. Le journal doit permettre de faire la
                # différence sans relire le playbook.
                "msg": str(brut.get("msg", ""))[:200],
            }
        )
    ...
    def v2_playbook_on_stats(self, stats: Any) -> None:
        ...
        # Fusionne plutôt que d'écraser : le lanceur peut jouer plusieurs
        # playbooks à la suite, chacun dans son propre processus, et le journal
        # doit dire ce que l'exercice entier a couvert.
