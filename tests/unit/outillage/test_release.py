...
def _sans_bruit(monkeypatch: pytest.MonkeyPatch, collection: Any) -> None:
    """Neutralise git, le changelog et la porte documentaire : chaque test ne juge que son refus."""
    ...
    # Même raison pour la porte documentaire : elle lit les pages du dépôt,
...
class _Collection:
    def __init__(self, version: str, chemin: Path) -> None:
        ...
        self.path = chemin
        self.fqcn = "stephrobert.outscale"
...
def test_des_fragments_en_attente_sont_refuses(
    ...
    _sans_bruit(monkeypatch, _Collection("0.1.0", tmp_path))
    monkeypatch.setattr(release, "fragments_en_attente", lambda _: ["first-release.yml"])
    refus = release.controler("0.1.0")
    assert len(refus) == 1
    assert "first-release.yml" in refus[0]
...
def test_une_documentation_impubliable_est_refusee(
    ...
    Les autres refus protègent la reproductibilité de l'archive. Celui-ci
    protège son lecteur : une page qui montre un filtre que l'API refuse ne
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            None,
            [release.docs_quality.Defaut("vm_info", "exemple-cle-de-filtre-inconnue", "Tags")],
        ),
    )
    refus = release.controler("0.1.0")
    assert len(refus) == 1
    assert "exemple-cle-de-filtre-inconnue" in refus[0]
...
def test_une_qualite_documentaire_non_mesurable_est_refusee(
    ...
    zéro défaut sur zéro page : un dossier vide passerait pour une
    documentation parfaite.
    """
    _sans_bruit(monkeypatch, _Collection("0.1.0", tmp_path))

    def _casse() -> None:
        raise release.docs_quality.QualiteError("aucune page examinée")
